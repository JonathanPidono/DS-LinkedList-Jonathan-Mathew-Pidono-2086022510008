...
Jawaban:
1. Head sebagai titik masuk (entry point) atau penanda node pertama dalam struktur data tersebut. Tanpa head, alamat memori node pertama akan hilang, menyebabkan seluruh linked list tidak dapat diakses.
...
6. Array. Akses elemen di posisi manapun adalah O(1) karena kita tinggal perlu menggunakan index langsung (arr[i]). Linked list harus melakukan traversal dari head satu per satu.
7. Linked list. Insertion di awal linked list adalah O(1) karena hanya memperbarui head dan satu pointer. Array membutuhkan O(n) karena semua elemen harus digeser ke kanan.
